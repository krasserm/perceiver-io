...
class ComputeEstimator:
    ...
    [3] https://github.com/karpathy/nanoGPT/blob/master/scaling_laws.ipynb)
    [4] https://news.ycombinator.com/item?id=34342427
    """
    ...
    def self_attn(self, num_channels, num_heads, num_layers):
        ...
        # This method is equivalent to `chinchilla_flops() in [3] divided by `num_latents`.

        attn_all = self._self_attn_layer(num_channels, num_heads) * num_layers
        mlp_all = self._mlp_layer(num_channels) * num_layers

        forward = attn_all + mlp_all
        forward_backward = forward * 3

        return forward_backward

    def cross_attn(self, num_channels, num_heads, prefix_dropout=0.5):
        ...
        attn_prefix = self._cross_attn_extra(num_channels, num_heads) * (1.0 - prefix_dropout)
    ...
    def _self_attn_layer(self, num_channels, num_heads):
        """Self-attention FLOPs per latent token per layer."""
        qkv = 6 * num_channels ** 2
        attn = 2 * num_channels * self.num_latents
        sm = 3 * num_heads * self.num_latents
        red = 2 * num_channels * self.num_latents
        out = 2 * num_channels ** 2
        return qkv + attn + sm + red + out

    def _cross_attn_extra(self, num_channels, num_heads):
        """Extra cross-attention FLOPs per latent token in first layer."""
        kv = 4 * num_channels ** 2 * self.num_prefix / self.num_latents
        attn = 2 * num_channels * self.num_prefix
        sm = 3 * num_heads * self.num_prefix
        red = 2 * num_channels * self.num_prefix
        return kv + attn + sm + red


class ModelInfo:
    def __init__(
        self, vocab_size: int, num_channels: int, num_layers: int, num_heads: int, compute_estimator: ComputeEstimator
    ):
        """
        :param num_channels: model dimension
        :param num_layers: number of self attention layers incl. hybrid layer
    ...
    def self_attn_flops(self):
        ...
        return self.compute_estimator.self_attn(
            num_channels=self.num_channels, num_layers=self.num_layers, num_heads=self.num_heads
        )

    def cross_attn_flops(self, prefix_dropout=0.5):
        """See `ComputeEstimator.cross_attn()`."""
        return self.compute_estimator.cross_attn(
            num_channels=self.num_channels, num_heads=self.num_heads, prefix_dropout=prefix_dropout
        )


def num_self_attn_params(num_channels, num_layers, num_latents, num_prefix, vocab_size):
    # Very close to chinchilla_params() in https://github.com/karpathy/nanoGPT/blob/master/scaling_laws.ipynb.
    # Not counting token embedding params but the final dense layer is equivalent to weight tying, which is
    # implemented in CausalLanguageModel. Hence, counting all parameters of a CausalLanguageModel is correct.
    # Main difference that chinchilla_params() counts parameters for learned relative position embeddings
    # whereas CausalLanguageModel learns absolute position embeddings. Rotary position embeddings do not add
