...
The following two sections describe how the conceptual architectures of Perceiver IO and Perceiver can be mapped to this
project's implementation architecture.

## Perceiver IO

![architecture-perceiver-io](images/architecture-perceiver-io.png)

Names of components shown in the implementation architecture are classes in [modules.py](../perceiver/model/core/modules.py).
Task-specific input and output adapters are subclasses of `InputAdapter` and `OuptutAdapter`, respectively, and provided
by task-specific modules of this project. Array dimensions (`M`, `C`), (`N`, `D`), (`O`, `F`)  and (`O`, `E`) have the
following names in code and/or on the command line:
...
Perceiver IO does **not** use repeated encoder cross-attention as described the [paper](https://arxiv.org/abs/2107.14795):
...
their weights. Sharing the weights with the first cross-attention layer can be configured with `first_cross_attention_layer_shared`,
sharing the weights with the first self-attention block can be configured with `first_self_attention_block_shared`. The
